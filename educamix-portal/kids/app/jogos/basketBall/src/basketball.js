...
function Basketball() {
	...
	this.sound = false;
	this.state = 'menu';
    this.menuText = new AnimatedText("Toque para Jogar!", 640 / 2, 960 / 2 + 50, 40, 0.01);
    this.overText = new AnimatedText("Toque para Continuar!", 640 / 2, 800, 40, 0.01);
    this.flashText = [];
    this.scored = 0;
    this.totalBalls = 3;
    this.round = 1;
	...
	this.update = function(delta) {
		if(this.state == 'menu') {
			if(this.click) {
				this.state = 'play';
                this.flashText.push(new FlashText("Iniciar!"));
				this.click = false;
			}
		...
		if(this.state == 'play') {
			...
			for(var i = 0; i < this.balls.length; i++) {
				...
				if(ball.y > 960) {
                    ...
                    if(!ball.scored) {
                        this.flashText.push(new FlashText("Errou!"));
                        if(++this.missed >= 3) {
                            this.state = 'over';
                        }
                    }
	...
	this.draw = function(ctx) {
		...
		if(this.state == 'menu') {
			...
			this.drawText(ctx, '(?) Toque para arremessar. Termina se errar 3 vezes.', 640 / 2, 940, 26);
			ctx.textAlign = 'left';
		}
		if(this.state == 'play') {
			for(var i = 0; i < this.hoops.length; i++) {
            ...
            ctx.textAlign = "left";
            this.drawText(ctx, "Pontos: " + this.score, 50, 70, 40);
            this.drawText(ctx, "Erros: " + this.missed, 460, 70, 40);
			for(var i = 0; i < this.texts.length; i++) {
				var text = this.texts[i];
				text.draw(ctx);
			}
		...
		if(this.state == 'over'){
			ctx.textAlign = 'center';
			this.drawText(ctx, 'Fim de Jogo', 640 / 2, 200, 80);
			this.drawText(ctx, 'Pontos: ' + this.displayScore, 640 / 2, 400, 50);
            if(this.storage) {
                this.drawText(ctx, "Melhor Pontuacao: " + localStorage.score, 640 / 2, 500, 50);
            }
