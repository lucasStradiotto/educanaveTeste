...
error_reporting(E_ALL);
    ...
    //     $uploadDirectory = 'uploads-new/'.$_POST["filename"].'.wav';
    ...
    //     else {
            // if it is video-blob
            if ( !empty( $_FILES ) ) {
                $uploadDirectory = 'uploads-new/'.$_POST["filename"].'.mp4';
                $tempPath = $_FILES[ 'file' ][ 'tmp_name' ];
                if (!move_uploaded_file($tempPath, $uploadDirectory)) {
                    echo("Problem writing video file to disk!");
                }
                else {
                    // $nome = time().''.rand(0,9);
                    // // $audioFile = 'uploads-new/'.$_POST["filename"].'.wav';
                    // $videoFile = 'uploads-new/'.$_POST["filename"].'.mp4';

                    // $mergedFile = 'uploads-new/'.$nome.'-merged.webm';
                    // $thumb = 'uploads-new/'.$nome.'-merged.jpeg';

                    $nome = $_POST["filename"];
                    // $audioFile = 'uploads-new/'.$_POST["filename"].'.wav';
                    $videoFile = $uploadDirectory;

                    $mergedFile = $uploadDirectory;
                    $thumb = 'uploads-new/'.$nome.'.jpeg';
                        ...
                        if($_REQUEST['act'] == 'enviado') {
                            ...
                            $put = array(
                                ...
                                'body'=>array(
                                    ...
                                    'tipo'=>'enviado',
                                    'videoId'=> 'http://educamix.com.br/kids/videomsg/'.$mergedFile,
                                    'visualizado'=>false,
                                    'data' => time()
                                )
                            );
